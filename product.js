...
function renderHero(d) {
    ...
    const prob = info["Stockout_Probability_Pct"];
    setText("hero-risk-note", prob != null ? `${Math.round(prob)}% chance of stockout` : "");
}
...
function renderOverview(d) {
    ...
    const probPct = info["Stockout_Probability_Pct"];
    setText(
        "kpi-risk-note",
        probPct != null
            ? `${n1(info["Days of Inventory"])} days left · ${Math.round(probPct)}% stockout chance`
            : `${n1(info["Days of Inventory"])} days of stock left`
    );
